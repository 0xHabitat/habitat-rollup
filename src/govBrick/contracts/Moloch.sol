...
contract Moloch {
  ...
  function onlyMember (address addr) internal {
    require(members[addr].shares > 0, 'Moloch::onlyMember - not a member');
  }

  function onlyDelegate (address addr) internal {
  ...
  ) internal
  {
    ...
    require(approvedToken.transferFrom(msgSender, address(this), proposalDeposit), 'Moloch::submitProposal - proposal deposit token transfer failed');
    ...
    */
    ...
    emit SubmitProposal(proposalIndex, msgSender, memberAddress, applicant, tokenTribute, sharesRequested);
  }
  ...
  function processProposal (address msgSender, uint256 proposalIndex) internal {
    ...
    // send msgSender the processingReward
    //require(
    //  approvedToken.transfer(msgSender, processingReward),
    //  'Moloch::processProposal - failed to send processing reward to msgSender'
    //);
  ...
  function ragequit(address msgSender, uint256 sharesToBurn) internal {
    ...
    //require(
    //  guildBank.withdraw(msgSender, sharesToBurn, initialTotalShares),
  ...
  function abort(address msgSender, uint256 proposalIndex) internal {
    ...
    emit Abort(proposalIndex, msgSender);
  }
  ...
  function getCurrentPeriod() public view returns (uint256) {
    return lastNow.sub(summoningTime).div(periodDuration);
  }
