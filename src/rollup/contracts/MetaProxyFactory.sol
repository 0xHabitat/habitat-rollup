...
contract MetaProxyFactory {
  ...
  /// @dev Creates a child with metadata from `metadata`.
  function _metaProxyFromBytes (bytes memory metadata) internal returns (address addr) {
    // the following assembly code (init code + contract code) constructs a metaproxy.
    assembly {
      // load free memory pointer as per solidity convention
      let start := mload(64)
      ...
      // now calculdate the size and copy the metadata
      let size := mload(metadata)
      // copy
      {
        let metaptr := add(metadata, 32)
        for { let i := 0 } lt(i, size) { i := add(i, 32) } {
          mstore(add(ptr, i), mload(add(metaptr, i)))
        }
      }
      ptr := add(ptr, size)
      // store the size of the metadata at the end of the bytecode
